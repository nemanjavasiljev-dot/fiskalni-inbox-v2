...
export async function GET(request: Request) {
  ...
  try {
    ...
    const ids = await getAuthorizedAccountantClientIds(admin, user.id);
    if (!ids.length) return NextResponse.json({ error: 'Nema aktivnih klijenata kojima imate pristup.' }, { status: 403 });
