...
export async function POST(request:Request){
  ...
  const {data:office}=await supabase.from('organizations').select('owner_user_id,organization_type').eq('id',officeId).maybeSingle();
  if(!office||office.organization_type!=='accounting'||office.owner_user_id!==user.id)return NextResponse.json({error:'Samo admin može dodeljivati klijente.'},{status:403});
