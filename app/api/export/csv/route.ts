...
export async function GET(request: Request) {
  ...
  const { data, error } = await supabase.from("receipts")
    .select("sdc_time,created_at,merchant_name,merchant_pib,invoice_number,total_amount,total_tax,raw_json,payment_method,buyer_pib,category,note,verification_status,qr_url,vat_deductible,vat_decided_at,ai_vat_recommendation,ai_vat_confidence")
  ...
  const rows = (data||[]).map((r:any)=>[
    r.sdc_time||r.created_at,r.merchant_name,r.merchant_pib,r.invoice_number,r.total_amount,receiptTotalTax(r),r.payment_method,r.buyer_pib,r.category,r.note,r.verification_status,r.qr_url,r.vat_deductible,r.vat_decided_at,r.ai_vat_recommendation,r.ai_vat_confidence
  ].map(cell).join(";"));
