...
export async function POST(request:Request){
  ...
  const payload={company_id:String(body.company_id||'')||null,company_name:companyName,pib,registration_number:String(body.registration_number||'').replace(/\D/g,'')||null,address:String(body.address||'').trim()||null,email:String(body.email||'').trim()||null,bank_account:String(body.bank_account||'').trim()||null,vat_rate:Number(body.vat_rate??20),is_demo:false,active:true,note:String(body.note||'').trim()||null,updated_at:new Date().toISOString()};
