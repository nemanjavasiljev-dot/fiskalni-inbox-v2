...
export async function POST(request:Request){
  ...
  const {data:issuer}=await ctx.admin.from("billing_issuer_settings").select("*").eq("active",true).order("created_at").limit(1).maybeSingle();
  const subtotal=unit*qty;const vatRate=20;const vat=Math.round(subtotal*vatRate)/100;const total=subtotal+vat;const prefix=type==="invoice"?"R":"P";const number=`FB-${prefix}-${new Date().toISOString().slice(0,10).replace(/-/g,'')}-${randomUUID().slice(0,6).toUpperCase()}`;
  ...
  const payload:any={organization_id:organizationId,invoice_number:number,document_type:type,plan,quantity:qty,unit_price_net:unit,subtotal_net:subtotal,vat_rate:vatRate,vat_amount:vat,total_amount:total,currency:"RSD",status:"unpaid",issued_at:new Date().toISOString(),due_at:body.due_at||null,recipient_name:org.name,recipient_pib:org.pib||null,recipient_address:org.address||null,recipient_email:email||null,issuer_snapshot:issuer||{company_name:"FiscalBox Demo",pib:"000000000",address:"Demo izdavalac",email:"billing@fiscalbox.local",vat_rate:20,is_demo:true,note:"DEMO izdavalac - zameniti produkcionim podacima."},note:String(body.note||"")||null};
