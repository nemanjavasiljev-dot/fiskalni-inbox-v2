...
export async function POST(){
  ...
  const [{data:orgs},{data:members},{data:profiles},{data:subscriptions}]=await Promise.all([
    ...
    ctx.admin.from("subscriptions").select("organization_id,status,provider,provider_subscription_id,ends_at,current_period_end")
  ]);
  const allOrgs:any[]=orgs||[];const allMembers:any[]=members||[];const subMap=new Map((subscriptions||[]).map((s:any)=>[String(s.organization_id),s]));
  ...
  function billable(orgId:string){const s:any=subMap.get(orgId);if(!s?.provider_subscription_id||s.provider!=="lemonsqueezy")return false;if(["active","paused","past_due"].includes(String(s.status)))return true;if(s.status==="cancelled")return new Date(s.ends_at||s.current_period_end||0).getTime()>Date.now();return false;}
  let count=0;
  for(const g of groups){
    const clientOrgIds: string[] = Array.from(new Set<string>(allMembers.filter((m:any)=>m.role==="accountant"&&g.memberIds.includes(String(m.user_id))).map((m:any)=>String(m.organization_id))));
    let appUsers=0;for(const orgId of clientOrgIds){if(companyMap.get(orgId)&&billable(orgId))appUsers+=usersPerOrg.get(orgId)||1;}
