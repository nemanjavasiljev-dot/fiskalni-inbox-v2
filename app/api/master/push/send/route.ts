...
export async function POST(request:Request){
  ...
  const [{data:members},{data:orgs}]=await Promise.all([ctx.admin.from("organization_members").select("organization_id,user_id,role").in("organization_id",ids),ctx.admin.from("organizations").select("id,company_id").in("id",ids)]);const userIds=Array.from(new Set((members||[]).filter((m:any)=>["owner","employee"].includes(m.role)).map((m:any)=>String(m.user_id))));const {data:subs}=userIds.length?await ctx.admin.from("push_subscriptions").select("*").in("user_id",userIds):{data:[] as any[]};
  ...
  await ctx.admin.from("master_notification_log").insert({channel:"push",target_organization_ids:ids,target_company_ids:(orgs||[]).map((o:any)=>o.company_id).filter(Boolean),subject:String(title),body:String(message),recipient_count:(subs||[]).length,sent_count:sent,created_by:ctx.user.id});
