...
export async function POST(request:Request){
  ...
  const {error:eventError}=await admin.from('billing_webhook_events').insert({provider:'lemonsqueezy',event_hash:hash,event_name:event,object_id:String(data?.id||''),payload});
  if(eventError && String(eventError.code)==='23505') return NextResponse.json({ok:true,duplicate:true});
  ...
  if(subscriptionEvents.has(event)){
    ...
    if(!organizationId) return NextResponse.json({ok:true,ignored:'missing_organization'});
    const plan=(planForVariant(attrs.variant_id)||String(custom.plan||'basic')) as FiscalBoxPlan;
    ...
    const update:any={
      ...
      last_webhook_at:new Date().toISOString(),activated_at:['on_trial','active'].includes(providerStatus)?new Date().toISOString():undefined
    };
    Object.keys(update).forEach(k=>update[k]===undefined&&delete update[k]);
    await admin.from('subscriptions').update(update).eq('organization_id',organizationId);
    await admin.from('organizations').update({plan,status:orgStatus(providerStatus,attrs.ends_at||null),trial_ends_at:attrs.trial_ends_at||null}).eq('id',organizationId);
  ...
  if(['subscription_payment_success','subscription_payment_failed','subscription_payment_recovered','subscription_payment_refunded'].includes(event)){
    ...
    if(!sub) return NextResponse.json({ok:true,ignored:'unknown_subscription'});
    ...
    if(!org) return NextResponse.json({ok:true,ignored:'missing_org'});
    ...
    const status=billingStatus(String(attrs.status|| (event==='subscription_payment_success'?'paid':'pending')));
    ...
    await admin.from('billing_invoices').upsert(invoice,{onConflict:'provider_invoice_id'});
    ...
    await admin.from('subscriptions').update(subUpdate).eq('id',sub.id);
    return NextResponse.json({ok:true});
  }
