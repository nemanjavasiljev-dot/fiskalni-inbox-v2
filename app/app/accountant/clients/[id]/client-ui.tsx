...
export default function ClientWorkspace({organization,selectedMonth,selectedYear,receipts,documents,receiptStatuses,documentStatuses}:any){
  ...
  const rawVat=monthReceipts.reduce((s:number,r:any)=>s+Number(receiptTotalTax(r)||0),0);
  const vat=monthReceipts.reduce((s:number,r:any)=>s+(r.vat_deductible===true?Number(receiptTotalTax(r)||0):0),0);
  const vatPending=monthReceipts.filter((r:any)=>Number(receiptTotalTax(r)||0)>0 && typeof r.vat_deductible!=="boolean").length;
