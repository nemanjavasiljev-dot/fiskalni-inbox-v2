...
export default async function AccountantSettingsPage(){
  ...
  const office:any={organization_id:officeMembership.organization_id,role:officeMembership.role,...officeMembership.organizations};
  const isAdmin=office.role==='owner'&&office.owner_user_id===user.id;
  ...
  if(isAdmin){
    const {data:sm}=await supabase.from('organization_members').select('user_id,role').eq('organization_id',office.organization_id).in('role',['owner','employee']);
    ...
    staff=(profiles||[]).map((p:any)=>({...p,office_role:roles.get(String(p.user_id))}));
