...
const statusLabel=(s:string)=>({paid:"PLAĆENO",unpaid:"NEPLAĆENO",cancelled:"STORNIRANO",refunded:"REFUNDIRANO",partial_refund:"DELIMIČNO REFUNDIRANO",converted:"REALIZOVAN PREDRAČUN"} as any)[s]||String(s||"").toUpperCase();
...
export default async function BillingPage({ searchParams }: { searchParams: Promise<{ status?: string }> }) {
  ...
  const content=<>
    <div className="app-head"><div><span className="pill"><FileText size={13}/> NAPLATA</span><h1>Moji računi</h1><p className="muted">Predračuni za uplatu i finalni računi nakon verifikovane bankarske uplate. Predračuni čekaju u Neplaćeno; finalni računi su u Plaćeno.</p></div><a className="btn btn-primary" href={`/app/subscription${accountingOrg?`?organization_id=${accountingOrg.organization_id}`:""}`}><CreditCard size={15}/> Pretplata</a></div>
    ...
    <div className="card billing-table"><div className="table-wrap"><table><thead><tr><th>Broj</th><th>Datum</th><th>Firma</th><th>Paket</th><th>Iznos</th><th>Status</th><th>Izvor</th><th>Dokument</th></tr></thead><tbody>{invoices.map((i:any)=><tr key={i.id}><td><b>{i.invoice_number}</b><div className="muted" style={{fontSize:10}}>{i.document_type==="proforma"?"Predračun":"Račun"}</div></td><td>{date(i.issued_at)}</td><td>{i.recipient_name}</td><td>{String(i.plan||"").toUpperCase()} · {i.quantity} korisnik(a)</td><td><b>{money(i.total_amount,i.currency)}</b><div className="muted" style={{fontSize:10}}>evidentirano</div></td><td><span className={`billing-status ${i.status}`}>{statusLabel(i.status)}</span></td><td>{i.provider==="bank_transfer"?"BANKARSKA UPLATA":i.provider==="lemonsqueezy"?"ONLINE PRETPLATA":"FISCALBOX"}</td><td><a className="btn" href={`/api/billing/invoices/${i.id}/pdf`} target={i.external_invoice_url?"_blank":undefined}><Download size={15}/> {i.external_invoice_url?"Račun":"PDF"}</a></td></tr>)}{!invoices.length&&<tr><td colSpan={8}><div className="empty-state">Još nema dokumenata naplate za izabrani filter.</div></td></tr>}</tbody></table></div></div>
  </>;
  return <div className={`app-shell ${isAccountant?"accountant-shell":""}`}><header className="appbar"><div className="container appbar-in"><a className="brand" href="/app"><span className="logo">F</span><BrandWordmark suffix={isAccountant?" · KNJIGO":""}/></a>{isAccountant?<div className="actions"><a className="btn" href="/app">← Nazad</a><form method="post" action="/api/auth/logout"><button className="btn">Odjava</button></form></div>:<CompanyHeaderMenu profile={profile} organization={companyOrg}/>}</div></header>{isAccountant?<div className="accountant-desktop-layout"><AccountantDesktopMenu isAdmin={isAdmin} username={profile.username||profile.full_name||profile.auth_email||""}/><main className="app-main accountant-main">{content}</main></div>:<main className="container app-main">{content}</main>}</div>;
}
