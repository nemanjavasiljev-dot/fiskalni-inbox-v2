...
export default function ActivateClient({token}:{token:string}){
  ...
  async function activate(e:React.FormEvent){
    ...
    setBusy(true);
    const r=await fetch('/api/client-invite/accept',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({token,password})});
  ...
  return <div className="invite-activate"><span className="pill">POZIV KLIJENTA</span><h1>Aktivirajte Fiskalni Inbox</h1><p className="muted"><b>{data.accounting_office}</b> je pripremio nalog za firmu <b>{data.company_name}</b>, PIB {data.pib}. Potrebno je samo da postavite lozinku.</p><form onSubmit={activate}><div className="field"><label>Email</label><input className="input" value={data.email||''} disabled/></div><div className="field"><label>Nova lozinka</label><input className="input" type="password" minLength={8} required value={password} onChange={e=>setPassword(e.target.value)} placeholder="Najmanje 8 znakova"/></div><div className="field"><label>Ponovite lozinku</label><input className="input" type="password" minLength={8} required value={confirm} onChange={e=>setConfirm(e.target.value)}/></div>{error&&<div className="error">{error}</div>}<button className="btn btn-primary" style={{width:'100%',marginTop:16}} disabled={busy}><KeyRound size={17}/>{busy?'Aktiviram…':'Postavi lozinku i aktiviraj nalog'}</button></form><div className="demo-box">Po aktivaciji dobijate probni paket od 10 dana i firma se automatski povezuje sa knjigovođom koji vas je pozvao.</div></div>;
}
