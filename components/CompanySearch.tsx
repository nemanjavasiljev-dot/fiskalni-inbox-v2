...
function fmtSync(v: string | null | undefined) {
  ...
  try {
    return new Intl.DateTimeFormat('sr-RS', { dateStyle: 'medium', timeStyle: 'short' }).format(new Date(v));
  } catch {
    return v;
  }
}
...
export default function CompanySearch({
  ...
  React.useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        ...
        setOpen(true);
        if (!(d.results || []).length) setMessage(d.warning || 'Nije pronađena firma. Proverite naziv ili matični broj.');
        else if (d.warning) setMessage(d.warning);
      } catch (e: any) {
        if (id === requestId.current) {
          setResults([]);
          setOpen(true);
          setMessage(e.message || 'Pretraga firmi trenutno nije dostupna. Pokušajte ponovo.');
        }
      } finally {
  ...
  async function refresh() {
    ...
    setMessage('');
    try {
      const r = await fetch(`/api/companies/${value.id}/refresh`, { method: 'POST' });
      const d = await r.json();
      if (!r.ok) throw new Error(d.error || 'APR osvežavanje nije uspelo.');
      onSelect(d.company);
      setQuery(d.company.name);
      setMessage('APR podaci su osveženi.');
    } catch (e: any) {
      setMessage(e.message || 'APR osvežavanje nije uspelo.');
    } finally {
      setRefreshing(false);
    }
  }
  ...
  return (
    <div className={`company-search ${compact ? 'compact' : ''} ${className}`}>
      ...
      {!value ? (
        ...
        <div className="company-search-selected">
      ...
      {open && !value && (
        <div className="company-search-dropdown">
          {loading && <div className="company-search-state"><Loader2 className="spin" size={17} /> Pretražujem APR bazu…</div>}
