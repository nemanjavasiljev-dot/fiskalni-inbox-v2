...
export default function CompanySearch({
  ...
  return <div className={`company-search ${compact?'compact':''} ${className}`}>
    ...
    {value&&showDetails&&<div className="company-search-details">
      <div><span>Naziv</span><b>{value.name}</b></div><div><span>Matični broj</span><b>{value.registration_number||'—'}</b></div><div><span>PIB</span><b>{value.pib||'—'}</b></div><div><span>Status</span><b>{value.registry_status||'—'}</b></div><div><span>Adresa</span><b>{value.address||'—'}</b></div><div><span>Mesto / opština</span><b>{[value.city,value.municipality].filter(Boolean).join(' · ')||'—'}</b></div><div><span>Delatnost</span><b>{[value.activity_code,value.activity_name].filter(Boolean).join(' · ')||'—'}</b></div><div><span>Pravna forma</span><b>{value.legal_form||'—'}</b></div>
      <div className="company-search-source"><span>Podaci iz APR-a · poslednja sinhronizacija: {fmtSync(value.apr_last_sync)}</span>{allowRefresh&&<button type="button" className="btn" onClick={refresh} disabled={refreshing}><RefreshCw size={14} className={refreshing?'spin':''}/>{refreshing?'Osvežavam…':'Osveži podatke'}</button>}</div>
