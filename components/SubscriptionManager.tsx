...
import { Landmark, ShieldCheck, ArrowUpCircle } from 'lucide-react';
...
export default function SubscriptionManager({organization,subscription,canManage=true}:{organization:any;subscription:any;canManage?:boolean}){
  ...
  const freeAccess=!bankActive&&!providerActive;
  async function selectPlan(plan:'basic'|'premium'){
    ...
    try{
      ...
      setMessage(d.message||'Predračun je kreiran i poslat.');window.location.href=d.url||`/app/files?org=${organization.id||organization.organization_id}&tab=billing`;
