...
export async function buildAccountantArchive(opts:{admin:any;accountantUserId:string;organizationIds:string[];type:'receipts'|'documents';month:string}){
  ...
  if(opts.type==='receipts'){
    ...
    const receipts=(rows||[]).filter((r:any)=>ym(r.sdc_time||r.created_at)===opts.month);
    ...
    const docs=(rows||[]).filter((d:any)=>ym(d.sent_at||d.created_at)===opts.month);const statusRows:any[]=[];
  ...
  if(!count)throw new Error(opts.type==='receipts'?'Nema primljenih računa za izabrani mesec.':'Nema primljenih dokumenata za izabrani mesec.');
